...
def main(args):
    ...
    # ====================================================
    # Step 3: Load the Expl Testing Data and Format Accordingly
    # ====================================================
    test_data_obj = ExplGenData(dataset = args.dataset, data_path = args.data_path, model_name = args.model_name, topK_path = args.pred_dir,chatgpt=True,maxlen=args.maxlen)
    test_expl_prompt_data = test_data_obj.generate_expl_dataset()
    
    del test_data_obj
    
    print("Total number of samples: ",len(test_expl_prompt_data))
    ...
    print("Expected #keys: ",len(test_expl_prompt_data.keys()))

    path_name = os.path.join(args.output_dir,args.dataset, f"test-keys.pkl")
    if os.path.exists(path_name):
        keys = load_pickle(path_name)
        print(f"Loaded from {path_name}")
    
    else:
        keys = list(test_expl_prompt_data.keys())[:]
        os.makedirs(os.path.join(f"{args.output_dir}", args.dataset), exist_ok=True)
        random.shuffle(keys)
        print("Generated after running data")
        save_pickle(keys,os.path.join(args.output_dir,args.dataset, f"test-keys.pkl"))
    ...
    for i in tqdm(range(start, end), desc="Generating explanations"):
        ...
        key = keys[i]
        pos_batch_prompts = test_expl_prompt_data[key]['pos_prompt']
        system_prompt_pos, description_pos = pos_batch_prompts[0], pos_batch_prompts[1]
        
        neg_batch_prompts = test_expl_prompt_data[key]['neg_prompt']
    ...
    file_name = os.path.join(f"{args.output_dir}", args.dataset, 'input-files-test', f'batch_{args.dataset}-start-{start}-end-{end}.jsonl')

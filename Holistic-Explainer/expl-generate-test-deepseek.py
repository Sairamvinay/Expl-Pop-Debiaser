...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default="generated-expl-deepseek/", help='Directory to save generated explanations')
    parser.add_argument("--pred_dir", type=str, default='../top_preds/', help='Directory to load the top-K recommendations dataset: such as ../top_preds/ which has files in form {model_name}-{dataset}-preds.pkl')
    parser.add_argument('--model_name', type=str, required=True, help='Name of the Model and condition: Eg: TallRec-Clean')
...
def main(args):
    ...
    # ====================================================
    data_obj = ExplGenData(dataset = args.dataset, data_path = args.data_path, topK_path= args.pred_dir,model_name = args.model_name)
    expl_prompt_data = data_obj.generate_expl_dataset()
    del data_obj
    print("Total number of samples: ",len(expl_prompt_data))
    # -------------------
    ...
    if args.debug:
        ...
        NUM_SAMPLES = len(expl_prompt_data.keys())
    
    keys = list(expl_prompt_data.keys())[:NUM_SAMPLES]
    random.shuffle(keys)
    ...
    if os.path.exists(save_path):
        ...
        print(f"Loaded Test Explanations from {save_path}: {len(explanations)}")
    else:
        explanations = {}
        print("Generating Test Explanations")
        ...
        for i in tqdm(range(0, len(keys), args.batch_size), desc="Generating explanations"):
            ...
            pos_batch_prompts = [expl_prompt_data[key]['pos_prompt'] for key in curr_keys]
            neg_batch_prompts = [expl_prompt_data[key]['neg_prompt'] for key in curr_keys]
            ...
            for key, pos_inp_ids,neg_inp_ids, pos_out_ids, neg_out_ids in zip(curr_keys,pos_enc['input_ids'],neg_enc['input_ids'], positive_outputs,negative_outputs):
                ...
                explanations[key] = {'pos-expl':clean_text(pos_text),'neg-expl':clean_text(neg_text),'label':expl_prompt_data[key]['label'],'target_item':expl_prompt_data[key]['target_item']}
        ...
        print(f"Saved Test Explanations to {save_path}: {len(explanations)}")
    ...
    del expl_prompt_data
        
    print("Generations Complete. Model saved.")

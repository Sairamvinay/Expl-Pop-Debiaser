...
from model_utils import DeepFM, bpr_loss
...
def main(args, train_loader, val_loader, user_num, item_num, local_rank):
    ...
    suffix = args.encoder_type
    
    check_step = 1
    
    output_dir = os.path.join(args.output_dir, args.dataset, f"encoder-{suffix}", f"stage-{args.stage}")
            ...
            with autocast(): 
                ...
                try:
                    if args.stage == 0:
                        ...
                        zero_emb = torch.zeros((len(user_ids), hidden_size), dtype=torch.float16).to(device) 

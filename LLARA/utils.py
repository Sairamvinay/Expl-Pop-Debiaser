...
def save_checkpoint(model, save_to, epoch, stage = -1):
    """
    Save the checkpoint at the current epoch. -1 means all
    """
    model_no_ddp = unwrap_dist_model(model)
    if stage == 1 or stage == -1:
        # save LoRA model
        save_checkpoint_peft(model_no_ddp.llama_model_lora,f"{save_to}/LORA/LORA_checkpoint_epoch_{epoch}.pt")
        safe_save_peft_adapter(model_no_ddp.llama_model_lora, save_to)
    
    if stage == 2 or stage == -1:
        # save Projection encoder
        save_obj = {
                   'projector':model_no_ddp.projector.state_dict(),
                   }
        
        torch.save(save_obj, f"{save_to}/PROJ/PROJ_checkpoint_epoch_{epoch}.pt")

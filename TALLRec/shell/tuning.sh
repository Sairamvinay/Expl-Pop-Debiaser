# Hyperparameter tuning for TALLRec Vanilla
# Replace dataset
...
python3 -u hyperparam_tuning-smaller.py --data_path $data_path --dataset $dataset --base_model $base_model --batch_size $batch_size --num_epochs $num_epochs --maxlen $maxlen --train_on_inputs --clip_grad_norm $clip_grad_norm --seed $seed --local_rank $local_rank --num_batches_train $num_batches_train --num_batches_val $num_batches_val >> outputs/$dataset-hp-smaller.txt
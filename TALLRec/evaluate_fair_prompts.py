...
from FairPromptData import get_fairprompt_loader
import argparse
...
def parse_eval_args():
    ...
    parser.add_argument('--dataset', type=str, required=True, help='Name of the dataset: beauty/toys/yelp/clothing')
    ...
    parser.add_argument("--model_name",type=str,default='TallRec-FAIR_PROMPTS',help='Name of Model to save the results: Such as TallRec-Clean')
...
def fair_prompts_evaluate_model(item2id, base_model, checkpoint_dir, data_path, dataset, batch_size, maxlen, gpu, load_8bit=False,num_workers=1,num_beams=1,model_name='TallRec-FAIR_PROMPTS',output_dir='outputs',seed = 999,temperature=0.7,top_p=0.9,top_k=50):
    seedSet(seed)
    args, _, _, values = inspect.getargvalues(inspect.currentframe())
    print("Arguments passed to fair_prompts_evaluate_model():")
    for arg in args:
        if arg != 'item2id':
    ...
    eval_loader, _ = get_fairprompt_loader(
        tokenizer, 
        popitems=popitems,
...
if __name__ == "__main__":
    ...
    evaluate_results = fair_prompts_evaluate_model(

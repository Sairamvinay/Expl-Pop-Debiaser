...
def get_dataset_loader(tokenizer, sample_numbers = 100,dataset='toys',data_path="../data",batch_size=16, workers=4, distributed=False,local_rank=0, shuffle=False):
    
    if dataset == 'yelp':
        data_obj = YelpTestData(tokenizer=tokenizer,sample_numbers=sample_numbers, dataset=dataset, data_path=data_path, local_rank=local_rank)
    else:
        data_obj = AmazonTestData(tokenizer=tokenizer,sample_numbers=sample_numbers, dataset=dataset, data_path=data_path, local_rank=local_rank)
...
class YelpTestData(Dataset):
    def __init__(self, tokenizer, sample_numbers = 100, dataset='toys', data_path = '../data/', local_rank=0):

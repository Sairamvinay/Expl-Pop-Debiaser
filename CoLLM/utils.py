...
def unwrap_dist_model(model):
    ...
    if hasattr(model, "module"):  # handles DDP after base_model nesting
        model = model.module
...
def safe_save_peft_adapter(peft_model, path):
    ...
    # Save adapter config
    peft_model.peft_config['default'].save_pretrained(path)
...
def save_checkpoint(model, save_to, epoch, stage = 1):
    """
    Save the checkpoint at the current epoch.
    """
    model_no_ddp = unwrap_dist_model(model)
    if stage == 1:
        # save LoRA model
        save_checkpoint_peft(model_no_ddp.llama_model_lora,f"{save_to}/checkpoint_epoch_{epoch}.pt")
        safe_save_peft_adapter(model_no_ddp.llama_model_lora, save_to)
    
    elif stage == 2:
        # save Rec encoder and Projection encoder
        save_obj = {'rec_encoder':model_no_ddp.rec_encoder.state_dict(),
                   'llama_proj':model_no_ddp.llama_proj.state_dict(),
                   }
        
        torch.save(save_obj, f"{save_to}/checkpoint_epoch_{epoch}.pt")

    print("Saving checkpoint at path {} at Epoch {}".format(save_to,epoch))

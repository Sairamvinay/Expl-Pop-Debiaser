...
def get_dataset_loader(dataset='toys',data_path="../data", sample_type='random',batch_size=16, workers=4, distributed=False,local_rank=0, shuffle=False):
    
    if dataset == 'yelp':
        data_obj = YelpData(dataset=dataset, data_path=data_path, batch_size=batch_size,sample_type=sample_type,local_rank=local_rank)
    
    else:
        data_obj = AmazonData(dataset=dataset, data_path=data_path,batch_size=batch_size, sample_type=sample_type,local_rank=local_rank)
...
class YelpData(Dataset):
    def __init__(self, dataset='toys',batch_size=100, data_path = '../data/', sample_type='random',local_rank=0):
    ...
    def __getitem__(self, idx):
        ...
        out_dict['InteractedItemTitles'] = ', '.join(purchase_history_titles)

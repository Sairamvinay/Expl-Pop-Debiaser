...
from utils import save_checkpoint,seedSet
...
def parse_args():
    ...
    parser.add_argument("--stage",type=int, required=True, help= "Which stage of model training: 1 (LoRA finetuning) or 2 (Recommender/Projector Training). If 2: please provide the stage 1 LoRA path in resume_on_checkpoint")
...
def train_custom(args):
    ...
    train_loader, train_dataset = get_dataset_loader(train_sample_number, mode='train', dataset=args.dataset, data_path=args.data_path, batch_size=args.batch_size, workers=args.num_workers, distributed=args.distributed,local_rank=args.local_rank)
    
    val_loader, val_dataset = get_dataset_loader(val_sample_number,mode='val', dataset=args.dataset, data_path=args.data_path, batch_size=args.batch_size, workers=args.num_workers, distributed=args.distributed,local_rank=args.local_rank)
    ...
    if args.distributed:
        ...
        VOCAB_SIZE = model.module.llama_model_lora.config.vocab_size
    else:
    ...
    optimizer,lr_scheduler = create_opt_lr(model,num_batches_train,args.gradient_accumulation_steps,num_epochs,args.warmup_ratio,args.learning_rate,args.weight_decay,adam_eps=1e-6)
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs): 
        ...
        if (not args.distributed) or (args.distributed and dist.get_rank() == 0):
            ...
            if avg_val_loss < best_val_loss:
                ...
                save_checkpoint(model, output_dir, "BEST", stage=args.stage)
                early_stopping_counter = 0
                terminate=False
            else:
        ...
        if terminate:
            save_checkpoint(model, output_dir, epoch, stage=args.stage)

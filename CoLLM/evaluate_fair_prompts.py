...
def parse_eval_args():
    ...
    parser.add_argument("--model_name",type=str,default='CoLLM-Clean',help='Name of Model to save the results: Such as CoLLM-Clean')
    ...
    parser.add_argument("--prompt_path",type=str, default="prompts/collm_amazon-fairprompts.txt",help='Path to load the prompt styles: e.g.: "prompts/collm_amazon-fairprompts.txt"')
...
def fair_prompts_evaluate_collm(seed=999,
                   gpu = 0,
                   dataset = 'beauty',
                   model_name = 'CoLLM',
                   data_path = '../data/',
                   ...
                   lora_dropout = 0.0,
                   project_mid=10,
                   llama_ckpt = 'snap/llama-beauty/stage-1/',
                   rec_ckpt = "snap/llama-beauty/stage-2/checkpoint_epoch_7.pt",
                   debug=False
                  ):
    ...
    print("Arguments passed to fair_prompts_evaluate_collm():")
    for arg in x:
    ...
    batch_size = 100
    eval_loader, eval_dataset = get_fairprompts_loader(popitems=popitems, dataset=dataset, data_path=data_path, batch_size=batch_size, workers=num_workers, shuffle = False, distributed=False,local_rank=gpu)    
    ...
    generation_kwargs = {"max_new_tokens":1,"temperature":args.temperature,"top_k":args.top_k, "top_p":args.top_p,"do_sample":True, "pad_token_id":model.llama_tokenizer.pad_token_id}
    ...
    with torch.no_grad():
        for stepv, batch in enumerate(tqdm(eval_loader)):
            ...
            beam_outputs['logits'] /= args.temperature
    ...
    popitems = eval_dataset.popitems # NEED TO ENSURE CORRECT MAPPING before evaluation
...
if __name__ == "__main__":
    ...
    evaluate_results = fair_prompts_evaluate_collm(seed=args.seed,
                   gpu = args.gpu,
